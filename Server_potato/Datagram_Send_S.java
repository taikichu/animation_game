...
//    static InetSocketAddress address_Port;
        Datagram_Send_S(){
            //framespeed
            String text = "framespeed " + BattleGui.framespeed;
            DatagramPacket sendPacket=new DatagramPacket(text.getBytes(),text.getBytes().length,new InetSocketAddress(Datagram_Receive_S.RehostIP,10009));//Datagram_Send_S.address_Port
            try {
            ...
            //color_count
            text = "color_count " + BattleGui.color_count;
            sendPacket=new DatagramPacket(text.getBytes(),text.getBytes().length,new InetSocketAddress(Datagram_Receive_S.RehostIP,10009));//Datagram_Send_S.address_Port
            try {
                Datagram_Send_S.socket_Client_10009.send(sendPacket);
            } catch (IOException e) {
                e.printStackTrace();
