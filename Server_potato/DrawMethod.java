	...
	public static void drawLine(int x1, int y1, int x2 , int y2 , Graphics g ) {
		if(field.equals("online")) {
			...
			for(String text : draw7_g) {
				DatagramPacket sendPacket=new DatagramPacket(text.getBytes(),text.getBytes().length,new InetSocketAddress(Datagram_Receive_S.RehostIP, 10008));//Datagram_Send_S.address_Port
//				sendPacket.setPort(Datagram_Send_S.SENDBACK_PORT);
	...
	public static void drawString(String line , int x , int y , int size , Graphics g) {
		if(field.equals("online")) {
			...
			draw7_g[6] = "null ";
			for(String text : draw7_g) {
				DatagramPacket sendPacket=new DatagramPacket(text.getBytes(),text.getBytes().length,new InetSocketAddress(Datagram_Receive_S.RehostIP, 10008));//
//				sendPacket.setPort(Datagram_Send_S.SENDBACK_PORT);
				try {
					Datagram_Send_S.socket_Client_10008.send(sendPacket);
				} catch (IOException e) {
